...
```
npm install -g localtunnel-remote
```
...
Assuming your local server is running on port 8000, just use the ```ltm``` command to start the tunnel.
...
You can restart your local server all you want, ```ltm``` is smart enough to detect this and reconnect once it is back.

### arguments

Below are some common arguments. See `ltm --help` for additional arguments
